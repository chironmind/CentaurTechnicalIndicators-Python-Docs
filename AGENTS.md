...
Documentation site for the public Python bindings (MkDocs / Material). This repo is
docs only — **do not edit library code here**.
...
mkdocs build     # the build is the pre-ship gate
```
...
Keep documentation examples aligned with the **current published Python package
behavior** — stale examples are a user-facing defect. Don't make cross-repo or
architectural decisions from inside this repo; surface them to the maintainer.
...
pick one and run. Beyond that, stop and report (don't work around) when the build
fails for reasons outside your change, or instructions conflict with the repo's
actual state.
